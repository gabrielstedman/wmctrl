...
static int set_desktop_name (Display *disp) {
    ...
    if (sscanf(options.param, "%d,%100[^\t\n]", &target, new_name) == 2) {
        ...
        names = g_malloc0( (desktop_list_size+strlen(new_name)) * sizeof(char));
        int idx[*num_desktops];
        idx[0] = 0;
        if (list) {
            id = 0;
        ...
        for(i=0; i<*num_desktops;i++){
            if((target-1)==i){
                strcat(names + tlen, new_name);
                tlen += strlen(new_name) + 1;
            }
            else{
                strcat(names + tlen, list + idx[i]);
                tlen += strlen(list + idx[i]) + 1;
            }
        }
